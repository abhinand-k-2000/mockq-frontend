...
const PaymentsDashboard = () => {
  ...
  const labels = transactionHistory.map(interview => new Date(interview.date).toLocaleDateString('en-IN', {day:'2-digit', month: 'short'}))
  const dataPoints = transactionHistory.map(interview => interview.price)
              ...
              <tbody>
                {currentTransactions.map((transaction, index) => (
                  <tr key={index} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">{transaction.candidate.name.toUpperCase()} </td>
        ...
        <div className="bg-white shadow-lg rounded-lg p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-gray-600">Email <a href="mailto:support@example.com" className="text-[#19328F] hover:underline">support@mockq.com</a></p>
            </div>
          </div>
        </div>
      </div>
