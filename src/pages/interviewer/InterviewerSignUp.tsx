...
const InterviewerSignUp = () => {
  ...
  const {register,formState: {errors}, handleSubmit} = useForm<IFormInput>({mode: "onChange"})
            ...
            <div className="w-full flex-1 mt-8">
              ...
              <div className="mx-auto max-w-xs flex flex-col gap-3">
                <input
                  ...
                  {...register("name", {required: true, pattern: /^(?=.*[a-zA-Z])[a-zA-Z ]{2,30}$/})}
                />
                 {errors.name && errors.name.type === "required" && (
                    <p  className="text-red-500 text-sm m-0 p-0">Name is required</p>
                  )}
                  {errors.name && errors.name.type === "pattern" && (
                    <p className="text-red-500 text-sm m-0 p-0">Invalid Name</p>
                  )}
                ...
                {errors.password && errors.password.type === "pattern" && (
                  <p className="text-red-500 text-sm">Password should be 6-16 characters long and contain at least one number and one special character</p>
                )}

                <button type="submit" className="mt-5 tracking-wide font-semibold bg-[#142057] text-gray-100 w-full py-4 rounded-lg hover:bg-[#19328F] transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none">
                  <svg
                ...
                <p className="mt-6 text-xs text-gray-600 text-center">
                  ...
                  <Link to="/interviewer/login">
                    <span className="text-[#142057] font-semibold">Sign in</span>
                  </Link>
                </p>
              </div>
              </form>
